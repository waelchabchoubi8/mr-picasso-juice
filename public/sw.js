...
self.addEventListener('fetch', (event) => {
  ...
  if (url.origin !== self.location.origin) return; // don't touch cross-origin (fonts, WhatsApp, etc.)
  ...
  if (request.mode === 'navigate') {
    event.respondWith(
      fetch(request)
        ...
        .catch(() => caches.match(request).then((r) => r || caches.match('/fr')))
    );
    return;
  }
